...
import {
  QueryUsedFixedAssetListDepartment,
  UpdateSelfFixedAssetPositionSelfDepartment,
  ApplyForFixedAssetReturnRecordDepartment,
  ApplyForFixedAssetAllocationRecordSelfDepartment,
...
export default defineComponent({
  ...
  setup: (props, ctx) => {
    ...
    const ReturnDialog = useDialog({
      ...
      on: {
        submit(row: any) {
          const UpdateContractTaxSubjectSmb = useRequest(ApplyForFixedAssetReturnRecordDepartment, {
            manual: true,
          });
    ...
    const TransferDialog = useDialog({
      ...
      on: {
        submit(row: any) {
          ApplyForFixedAssetAllocationRecordSelfDepartment(row).then(res => {
            if (res.data.success) {
              ctx.root.$message.success('操作成功');
              query.reload();
            } else {
    ...
    const query = usePagination(QueryUsedFixedAssetListDepartment, {
      defaultPageSize: 50,
    });
