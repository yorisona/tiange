  ...
  >
    <div class="btns-line">
      <!-- <tg-button
        ...
        icon="ico-btn-add"
        @click="openDrawer"
        v-if="Permission.canChange && hasPermission"
        >业绩登记</tg-button
      > -->
      <label>登记收款：</label>
      <span>{{ moneyFormat(statInfo.total_gather_amount) }}</span>
      <label class="mgl-18">确认收款：</label>
      <span>{{ moneyFormat(statInfo.confirmed_gather_amount) }}</span>
      <label class="mgl-18">等待确认：</label>
      <span>{{ moneyFormat(statInfo.wait_gather_amount) }}</span>
      <label class="mgl-18">已核销金额：</label>
      <span>{{ moneyFormat(statInfo.write_off_amount) }}</span>
      <label class="mgl-18">未核销金额：</label>
      <span>{{ moneyFormat(statInfo.not_write_off_amount) }}</span>
    ...
    <refundDialog
      ...
      :row="refundRow"
      @dialog:close="onRefundDialogClose"
    />
    <InvoicesDetail
      ref="dialogRef"
    ...
    <refund-write-off
      type="pay"
      :notWriteOffAmount="refundWriteOffAmount"
      :refund_cost_id="refundWriteOffId"
      :achievement_id="refundWriteOffAchievementId"
      :visible="refundWriteOffVisible"
      @cancel="() => (refundWriteOffVisible = false)"
...
<script src="./achievement.ts"></script>
