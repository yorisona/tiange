    ...
    >
      ...
      <div class="detail-wrapper">
        <div class="left-wrapper">
          ...
          <div class="left-body">
            <div class="line" v-if="textType !== '领用'">
              <p class="label">报废原因：</p>
              <!-- <div class="value">
                  {{ baseData.cooperation_company_name }}
                </div> -->
              <div class="value line-value">{{ endTypeStr() }}</div>
            </div>
            <div class="line" style="height: auto" v-if="textType !== '领用'">
              <p class="label">报废说明：</p>
              <div class="value line-value">
                {{ baseData.approval_detail.scrapped_explain || '--' }}
              </div>
            </div>
            ...
            <div class="line" style="height: auto">
              ...
              <div class="value line-value">
                <template
                  v-if="
                    baseData.approval_detail &&
                    baseData.approval_detail.file_urls &&
                    baseData.approval_detail.file_urls.length
                  "
                >
                  <Appendix :list="baseData.approval_detail.file_urls" />
                </template>
                <span v-else>--</span>
              </div>
            </div>
            <div class="line" style="height: auto; width: 95%">
              <p class="label">{{ textType }}明细：</p>
              <tg-table
                border
                class="mgb-6"
                :data="
                  baseData.level_two_types === 2
                    ? baseData.approval_detail.receive_relation_list
                    : baseData.approval_detail.scrapped_relation_list
                "
                :columns="columns"
              >
              </tg-table>
