...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const textType = computed(() => {
      return baseData.value.level_two_types === 2 ? '领用' : '报废';
    });
    ...
    const endTypeStr = () => {
      const end_str = E.fixedassets.reasonForScrappingMap.get(
        baseData.value.approval_detail.scrapped_reason,
      );
      return end_str ?? '--';
    };
    ...
    const columns = computed(() => {
      if (baseData.value.level_two_types === 6) {
        return [
          {
            label: '资产编号',
            prop: 'asset_code',
