...
import { GetPaymentList } from '@/services/finance/payment';
import { BankAccount, CostTypeMap, FinancePayment } from '@/types/tiange/finance/finance';
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    /** 编号最大长度 */
    const uid_max_length = max_length_of_column(list, '编号', uid_render);

    /** 付款编号渲染函数 */
    const pay_id_render = <T extends boolean>(row: FinancePayment, text_only: T) =>
      text_only
        ? row.approval_cost_record_id || '--'
        : (h(
            'span',
            { class: row.reverse_id !== null || row.reversed_id !== null ? 'reverse-red' : '' },
            [row.approval_cost_record_id || '--'],
    ...
    /** 付款编号最大长度 */
    const pay_id_max_length = max_length_of_column(list, '付款编号', pay_id_render);
    ...
    const columns = computed<TableColumn<FinancePayment>[]>(() => [
      {
        ...
        fixed: 'left',
        align: 'center',
        minWidth: uid_max_length.value < 90 ? 90 : uid_max_length.value,
      ...
      },
      {
        ...
        fixed: 'left',
        align: 'center',
        minWidth: pay_id_max_length.value < 90 ? 90 : pay_id_max_length.value,
        formatter: row => pay_id_render(row, false),
      },
      {
        align: 'right',
        label: '付款金额 (元)',
        minWidth: pay_amount_max_length.value < 120 ? 120 : pay_amount_max_length.value,
    ...
    const getOurBankAccounts = async () => {
    ...
    const accountDate = (row: FinancePayment) => {
  ...
  methods: {
    ...
    getParams() {
      ...
      return {
        ...
        payer_bank_account_id: query.payer_bank_account_id,
