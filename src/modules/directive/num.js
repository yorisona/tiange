...
const optionCheck = binding => {
  // 范围值是否为数值
  if (
    (binding.value.max && typeof binding.value.max !== 'number') ||
    (binding.value.min && typeof binding.value.min !== 'number')
  ) {
    throw new Error('范围参数必须是数字');
  }
  // 最大最小值存在的时候判断最大值与最小值是否相等
  if (
    binding.value.max === binding.value.min &&
    typeof binding.value.max !== 'undefined' &&
    typeof binding.value.min !== 'undefined'
  ) {
...
const isInvalidVal = bindValue => {
  /*bindValue.toString().indexOf('.') === bindValue.length - 1 ||*/
  return (
    bindValue === null ||
    bindValue === '' ||
    isNaN(Number(bindValue)) ||
    bindValue.toString().indexOf('e') !== -1
  );
};
...
export default {
  ...
  inserted(el, binding, vNode) {
    /** 默认输两位小数 **/
    binding.value.precision = binding.value.precision >= 0 ? binding.value.precision : 2;
    let content = '';
    let oldContent = '';
    ...
    el.addEventListener('keyup', e => {
      ...
      if (
        bindValue === null ||
        inputReg.test(bindValue) === false ||
        String(bindValue || '').indexOf('NaN') >= 0 ||
        isNaN(bindValue)
      ) {
        setVal(null, el, vNode);
        return;
      ...
      if ((arr.length > 1 && arr[1].length < 1) || binding.value.precision === 0) {
        oldContent = inputVal;
        setVal(inputVal, el, vNode);
        return;
      }
      if (binding.value.precision && arr.length > 1 && arr[1].length > binding.value.precision) {
        setVal(oldContent, el, vNode);
        return;
      }
      content = parseFloat(inputVal);
      const contentStr = String(content);
      if (
        contentStr.indexOf('.') >= 0 &&
        contentStr.split('.')[1].length > binding.value.precision
      ) {
    ...
    el.addEventListener('focusout', e => {
      ...
      // 处理无效值
      const bindValue = e.target.value;
      if (isInvalidVal(bindValue) || bindValue === null) {
        setVal(null, el, vNode);
        return;
      }
      const arr = String(e.target.value).split('.');
      if ((arr.length > 1 && arr[1].length < 1) || binding.value.precision === 0) {
        setVal(arr[0], el, vNode);
        return;
      }
