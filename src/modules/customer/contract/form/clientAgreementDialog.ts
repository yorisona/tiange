...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const form = ref<any>({
      ...
      need_pre_day: undefined,
      need_pay_money: undefined,
      expiration_date_two: undefined,
      expiration_date_one: undefined,
      contract_xieyi_pintai_one_type: undefined,
      contract_xieyi_pintai_two_type: undefined,
      contract_xieyi_pintai_three_type: undefined,
      supportValue_all: undefined,
      supportValue_one: undefined,
      supportValue_two: undefined,
      supportValue_three: undefined,
      supportValue_four: undefined,
      supportValue_pingtai: undefined,
      contract_xieyi_three_type: true,
      contract_xieyi_two_type: true,
      contract_xieyi_one_type: true,
    ...
    const formRules = ref({
      ...
      cooperation_gather_date: [
        { required: true, message: '请输入直播日期（暂定）', trigger: ['blur', 'change'] },
      ],
      cooperation_work_name: [
        { required: true, message: '请输入产品服务/服务名称', trigger: ['blur', 'change'] },
      ],
      ...
      payee_request_type: [{ required: true, message: '请选择收款要求', trigger: 'change' }],
      ...
      cooperation_zhubo_real_last_time: [
        { required: true, message: '请选择最后一期款项收款时间', trigger: 'change' },
      ],
    ...
    watch(
      ...
      form => {
        ...
        if (
          ...
          (formValue.payee_type === 2 && formValue.payee_money_two) ||
          formValue.payee_type === 3 ||
          (formValue.payee_type === 4 && formValue.payee_remarkstr)
        ) {
          setTimeout(() => {
    ...
    watch(
      () => {
        return form.value.contract_xieyi_pintai_one_type;
      },
      contract_xieyi_pintai_one_type => {
        if (contract_xieyi_pintai_one_type === true) {
          form.value.contract_xieyi_pintai_two_type = false;
          form.value.contract_xieyi_pintai_three_type = false;
        }
      },
      {
        deep: true,
    ...
    watch(
      () => {
        return form.value.contract_xieyi_pintai_two_type;
      },
      contract_xieyi_pintai_two_type => {
        if (contract_xieyi_pintai_two_type === true) {
          form.value.contract_xieyi_pintai_one_type = false;
          form.value.contract_xieyi_pintai_three_type = false;
        }
      },
      {
    ...
    watch(
      () => {
        return form.value.contract_xieyi_pintai_three_type;
      },
      contract_xieyi_pintai_three_type => {
        if (contract_xieyi_pintai_three_type === true) {
          form.value.contract_xieyi_pintai_two_type = false;
          form.value.contract_xieyi_pintai_one_type = false;
        }
      },
    ...
    const onSupplierIdChange = (id: string) => {
      const idstr: any = id;
      frame_contract_uid.value = idstr;
    };
    // 搜索获取供应商列表
    ...
    const onSaveBtnClick = async (btntype: any) => {
      const formValue = form.value;
      if (formValue.cashdeposit_need_type) {
        if (
          !formValue.cashdeposit_need_date ||
          !formValue.cashdeposit_need_money ||
          !formValue.cashdeposit_need_day
        ) {
      ...
      if (formValue.contract_status === 1) {
        ...
        if (formValue.contract_xieyi_two_type) {
          if (
            !formValue.supportValue_one ||
            !formValue.supportValue_two ||
            !formValue.supportValue_all ||
            !formValue.supportValue_three ||
            !formValue.supportValue_four ||
            (formValue.contract_xieyi_pintai_three_type && !formValue.supportValue_pingtai)
          ) {
            ctx.root.$message.warning('请将保价条款填写全');
            return false;
          }
          if (
            !formValue.contract_xieyi_pintai_one_type &&
            !formValue.contract_xieyi_pintai_two_type &&
            !formValue.contract_xieyi_pintai_three_type
          ) {
            ctx.root.$message.warning('请将保价条款填写全');
            return false;
          }
        }
        if (formValue.contract_xieyi_three_type) {
          if (!formValue.expiration_date_one || !formValue.expiration_date_two) {
            ctx.root.$message.warning('请将保质期约定条款填写全');
            return false;
          }
        }
        if (
          !formValue.need_pre_day ||
          !formValue.need_pay_money ||
          !formValue.live_day_pre ||
          !formValue.live_day_later ||
          !formValue.refund_day ||
          !formValue.live_start_day_pre
        ) {
      ...
      if (result) {
        ...
        let pay_condition = [
          ...
          },
          {
            ...
            value:
              formValue.payee_type === 3
                ? formValue.payee_yongjinlv_four || 0
                : formValue.payee_type === 2
                ? formValue.payee_yongjinlv_two || 0
                : undefined,
          },
          {
            key: '其它',
            value: formValue.payee_type === 4 ? formValue.payee_remarkstr : undefined,
          },
          {
            key: 'ROI要求',
            value:
          ...
          {
            key: '执行要求',
            value: formValue.payee_execute_type === 1 ? '先执行后收款' : '先收款后执行',
          },
          {
            key: '付款要求',
            value: formValue.payee_request_type === 1 ? '分期收款' : '一次性收款',
          },
          {
            key: '票款要求',
        ...
        if (formValue.contract_status === 1) {
          others = others.concat(
            ...
            {
              key: '直播开始前日期',
              value: formValue.live_start_day_pre,
            },
            {
              key: '大主播合作条款',
              value: formValue.contract_xieyi_one_type ? 1 : 0,
            },
            {
              key: '保价条款',
              value: formValue.contract_xieyi_two_type ? 1 : 0,
            },
            {
              key: '直播前自然日',
              value: formValue.contract_xieyi_two_type ? formValue.supportValue_one : undefined,
            },
            {
              key: '直播后自然日',
              value: formValue.contract_xieyi_two_type ? formValue.supportValue_two : undefined,
            },
            {
              key: '共直播自然日',
              value: formValue.contract_xieyi_two_type ? formValue.supportValue_all : undefined,
            },
            {
              key: '直播选择平台',
              value: formValue.contract_xieyi_two_type
                ? formValue.contract_xieyi_pintai_one_type
                  ? '所有网络平台'
                  : formValue.contract_xieyi_pintai_two_type
                  ? '所有线上线下平台'
                  : formValue.contract_xieyi_pintai_three_type
                  ? formValue.supportValue_pingtai
                  : undefined
                : undefined,
            },
            {
              key: '退还差价倍数',
              value: formValue.contract_xieyi_two_type ? formValue.supportValue_three : undefined,
            },
            {
              key: '赔偿违约金',
              value: formValue.contract_xieyi_two_type ? formValue.supportValue_four : undefined,
            },
            {
              key: '保质期约定',
              value: formValue.contract_xieyi_three_type ? 1 : 0,
            },
            {
              key: '保质期约定赔偿差价倍数',
              value: formValue.contract_xieyi_three_type
                ? formValue.expiration_date_one
                : undefined,
            },
            {
              key: '保质期约定赔偿乙方金额',
              value: formValue.contract_xieyi_three_type
                ? formValue.expiration_date_two
                : undefined,
            },
            {
              key: '第四项甲方赔偿乙方金额',
              value: formValue.need_pay_money,
            },
            {
              key: '甲方提前通知乙方天数',
              value: formValue.need_pre_day,
            },
            {
              key: '诉讼方',
              value: formValue.contract_xieyi_yifang_type ? '乙方' : '原告',
            },
          );
        }
        const payload: any = {
          ...
          margin: margin,
          others: others,
          customer_id: frame_contract_uid.value,
        ...
        if (btntype === 1) {
          ...
          const postData: any = {
            contract_agreement_type: 'customer',
            contract_uid: contract_uid.value,
            dataset: {
              project_id: my_project_id.value || project_id.value,
              contract_uid: contract_uid.value,
      ...
      Promise.resolve()
        ...
        .then(res => {
          if (res.data.success) {
            ...
            ctx.root.$message.error(res.data.data);
          }
        })
        .catch(() => {
  ...
  methods: {
    ...
    show(item: any) {
      ...
      if (item) {
        ...
        this.frame_contract_uid = item.template_info.customer_id || item.contract_info.company_id;
        this.file_url_list =
