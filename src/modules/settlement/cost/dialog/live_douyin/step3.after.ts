...
import { calcTotalAdjustAmount, commonForm } from '../shoplive/utils';
import {
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    /** 下载投放明显 */
    const downloadPutServiceFeeFile = () => {
      // const router = useRouter();
      // const project_id: any = router.currentRoute.params.id;
      const settlement_project_id = settlement.value?.project_id;
      window.open(
        put_yf_detail_url(
    ...
    const previewPutServiceFeeFile = () => {
      const settlement_id = settlement.value ? settlement.value.id.toString() : '-1';
      const router = useRouter();
      const settlement_project_id: any = router.currentRoute.params.id;
      const start_date = settlement.value?.start_date ?? 0;
      const end_date = settlement.value?.end_date ?? 0;
      const url = `/api/oceanengine/ad_plan/download_ad_cost/yufeng?project_id=${settlement_project_id}&start_date=${start_date}&end_date=${end_date}&settlement_id=${settlement_id}&Authorization=${JWT_TOKEN}`;
      previewFile(url);
    };
    ...
    const submit = async () => {
      ...
      if (settlement.value.is_estimate === 0 && DataForm.value.seal_type === null) {
        ctx.root.$message.warning('请选择是否盖章');
        return;
      }
      if (!settlement.value.is_estimate && !cooperation_link_contract_id.value) {
        ctx.root.$message.warning('请选择关联合同');
        return;
      }
      if (
    ...
    const kol_service_all_amount_str = computed(() => {
      return Decimal2String(
        new Decimal(
          Number(DataForm.value.kol_service_amount || 0) +
            Number(DataForm.value.company_service_amount || 0),
        ),
      );
    });
    ...
    const getAdjustAmount = computed(() => {
      if (DataForm.value.adjust_info && DataForm.value.adjust_info.length > 0) {
        let amount: any = 0;
        DataForm.value.adjust_info.map((item: any) => {
        ...
        return Decimal2String(new Decimal(amount || 0));
      }
      return 0;
    });
    //根据成本类型获取信息
    const getDetailByType = (type: any) => {
      return DataForm.value.adjust_info.filter((el: any) => el.type === type);
    };
    ...
    return {
      ...
      downloadPutServiceFeeFile,
      previewPutServiceFeeFile,
