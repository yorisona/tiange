...
import otherIncome from '../../../component/incomeDetail/other.income.vue';
import {
  AdjustInfo,
  CompanyInfo,
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const dataForm = ref<MCNTaobaoOtherForm>({
    ...
    const originDataForm = ref<MCNTaobaoOtherForm>(deepClone(dataForm.value) as MCNTaobaoOtherForm);
        ...
        dataForm.value.company_info_list?.forEach(element => {
          switch (element.type) {
            case SettlementIncomeType.other: {
              ...
              if (not_complete) {
                income_ok = false;
                income_error_message = '请完善成本数据信息';
              }
              break;
            }
            default:
    ...
    onMounted(() => {
      originDataForm.value = deepClone(dataForm.value) as MCNTaobaoOtherForm;
    });
  ...
  render() {
    return (
      <settlement-step2-layout
        ...
        class="tg-cost-mcn-taobao-other-step2-before-container"
      >
        <top-card slot="top" amount={this.total_settlement_amount} type="default"></top-card>
        <div class="left-content" slot="left">
          <div class="income-info">
            <div class="income-info-title">总结算金额 = 填写的所有成本金额合计</div>
            <other-income
              feeType={2}
              feeDetail={this.dataForm.company_info_list?.[0]}
            ></other-income>
          </div>
        </div>
