...
import { Decimal2String, formatAmount, formatAmountWithoutPrefix } from '@/utils/string';
import {
  ...
  setup(props, ctx) {
    ...
    const methods = {
      ...
      next: async () => {
        ...
        if (!cooperation_link_contract_id.value && !settlement.value?.is_estimate) {
      ...
      isModified: () => {
        ...
        if (
          ...
          seal_type.value !== old_seal_type.value
        ) {
          return true;
        }
