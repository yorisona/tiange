...
export default defineComponent({
  ...
  setup(props, ctx) {
          ...
          switch (element.type) {
            ...
            case SettlementIncomeType.talent_cost: {
              const not_complete = element.company_list?.find(
                el => !el.company_id || !el.kol_id || !el.income_amount,
              );
              if (not_complete) {
                income_ok = false;
                if (element.type === SettlementIncomeType.put) {
                  income_error_message = '请完善投放成本数据信息';
                } else if (element.type === SettlementIncomeType.talent_cost) {
            ...
            case SettlementIncomeType.other: {
              ...
              if (
                ...
                } else if (element.type === SettlementIncomeType.xingtu) {
    ...
    const onDeleteBtnClick = async (
      ...
      const adjust_info = dataForm.value.adjustInfo.filter(item => {
        return item.adjust_amount || item.adjust_reason;
      });
      let isfind = false;
      ...
      adjust_info.map((item: any) => {
        dataForm.value.company_info_list?.forEach(settlementDetailEl => {
          if (settlementDetailEl.company_name) {
            if (item.company_name === settlementDetailEl.company_name) {
              isfind = true;
            }
          } else {
            const finder = settlementDetailEl.company_list?.find(el => {
              return el.company_name === item.company_name;
            });
            if (finder) {
              isfind = true;
            }
