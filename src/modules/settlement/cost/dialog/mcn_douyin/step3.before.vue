<template>
  <SettlementStep2Layout
    class="tg-cost-mcn-douyin-step3-before-container"
        ...
        >
          ...
          >
            <div class="before-income-row-detail">
              ...
              <tg-button
                class="down-load"
                v-if="item.type === 8 && isYuFengCompany(company.company_name)"
                type="link"
                @click="() => onDownloadPutDetail(item.type, company.amount_info_list)"
                >下载明细</tg-button
              >
            </div>
            <div
              class="other-desc"
              v-if="item.type !== 1 && remark(item.type, company.amount_info_list)"
            >
...
.tg-cost-mcn-douyin-step3-before-container {
  .step2-cost-layout-content {
    grid-template-rows: 24px 470px !important;
  }
