import {
  QueryProjectScheduleCalendar,
  ScheduleCancelRestShopLive,
  ScheduleSetRestShopLive,
} from '@/services/live';
  ...
  setup(props, ctx) {
    ...
    const methods = {
      ...
      onRest: async (day: CalednarData) => {
        ...
        if (day.status === 1) {
          //  取消休息
          const res = await ScheduleCancelRestShopLive(
            day.shop_live_list[0].id,
            business_type.value,
          );
          if (res.data.success) {
        ...
        //  设置休息
        const res = await ScheduleSetRestShopLive(project_id, day.date, business_type.value);
        if (res.data.success) {
          methods.queryProjectScheduleCalendar();
      ...
      queryProjectScheduleCalendar: async () => {
        ...
        const [res, _] = await Promise.all([
          await QueryProjectScheduleCalendar(params, business_type.value),
          await sleep(500),
        ]);
      ...
      three_live: (live_data: ProjectScheduleCalendarData) => {
        return live_data.shop_live_list.filter((_, index) => index < 3);
      },
      is_live_abnormal: (live: ProjectScheduleLiveData) => {
        let is_abnormal = false;
        if (!live.is_kol_scheduled || !live.is_entry_live_data) {
          is_abnormal = true;
        }
        return is_abnormal;
      },
      abnormal_live_tips: (live: ProjectScheduleLiveData) => {
        const tips = [];
        if (!live.is_kol_scheduled) {
          tips.push(`${tips.length + 1}、场次未完成主播排班`);
        }
        if (!live.is_entry_live_data) {
          tips.push(`${tips.length + 1}、已过直播结束时间仍未录入直播数据`);
        }
        return tips;
      },
      // 调转详情
