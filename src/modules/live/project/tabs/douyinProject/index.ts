...
import { defineComponent, computed, h, ref, inject, Ref, watchEffect } from '@vue/composition-api';
...
import weiguanlianzhuboImage from '@/assets/img/projectDetail/icon-local-life-weiguanlianzhubo.png';
  ...
  setup(props, ctx) {
    ...
    const reqQueryReminds = useRequest(QueryInProjectReminds, {
      ...
      onSuccess(data) {
        ...
        if (remindsData.value?.data_update_time) {
          remindsData.value.data_update_time = moment(remindsData.value.data_update_time).format(
            'yyyy.MM.DD HH:mm:ss',
          );
        }
      },
    ...
    const onPageItemHandler = (name: string, warning?: string, otherParams?: any) => {
      if (warning) {
