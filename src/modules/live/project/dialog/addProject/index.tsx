...
const useArtificial = (ctx: SetupContext) => {
  ...
  const submit = async (business_type: number | undefined, isSingleField = false) => {
    ...
    delete _data.estimated_data_list;
    _data.local_life_type = 2; //1: 常规直播项目 2: 单次营销大场
    if (isSingleField === false) {
      _data.local_life_type = business_type === E.project.BusinessType.locallife ? 1 : undefined;
      _data.supply_chain_type =
        business_type === E.project.BusinessType.supplyChain ? 1 : undefined;
    ...
    const { data: response } = await ShopLiveProjectApproval(_data as any, business_type);
    if (response.success) {
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const artForm = useArtificial(ctx);
    artForm.business_type = props.businessType !== 0 ? props.businessType : artForm.business_type;
    const treeRef = ref();
    const saveLoading = ref(false);
    const steps = reactive({
      step: 1,
      cleanText: '取消',
      okText: props.isSingleField ? '保存' : '下一步',
      clean: () => {
        if (steps.step === 2) {
          steps.step = 1;
          steps.okText = '下一步';
      ...
      submit: () => {
        if (steps.step === 1) {
          Promise.resolve()
            ...
            .then(async () => {
              if (props.isSingleField) {
                saveLoading.value = true;
                await artForm.submit(artForm.business_type, props.isSingleField);
          ...
          Promise.resolve()
            .then(() => {
              return new Promise((resolve, reject) => {
                ...
                saveLoading.value = true;
                await artForm.submit(artForm.business_type);
                saveLoading.value = false;
                ctx.emit('close');
                ctx.emit('submit');
              });
    ...
    watch(
      ...
      () => {
        artForm.business_type = artForm.business_type === 8 ? undefined : artForm.business_type;
      },
    );
    return {
      saveLoading,
  ...
  render() {
    ...
    return (
      <div class="dialog-import">
        {this.steps.step === 1 && this.isSingleField === false && (
        ...
        <div class="footer">
          ...
          <tg-button v-loading={this.saveLoading} onClick={this.steps.submit} type="primary">
            {this.steps.okText}
          </tg-button>
        </div>
      </div>
