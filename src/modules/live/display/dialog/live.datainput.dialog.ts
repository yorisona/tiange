/*
 * @Brief: 店铺代播-场次详情-直播数据录入
 * @author  wuyou <wuyou@goumee.com>
 * @since   2021-01-15 11:03:23
 */
...
import { defineComponent, PropType, ref, Ref, watch, inject } from '@vue/composition-api';
import { HttpRequestOptions } from 'element-ui/types/upload';
import lodash from '@/utils/lodash/custom';
import { DisplayLiveDataSave } from '@/services/live';
  ...
  setup(props, ctx) {
    ...
    watch(
      ...
      newVal => {
        ...
        if (newVal) {
          DisplayDataForm.value.id = props.DisplayInfo?.id ?? -1;
          ...
          DisplayTimeImageUrl.value =
            DisplayDataForm.value.duration_screenshot + `?Authorization=${getToken()}`;
          DisplayDataImageUrl.value =
            DisplayDataForm.value.data_screenshot + `?Authorization=${getToken()}`;
    ...
    const submit = async () => {
      ...
      const { data: response } = await DisplayLiveDataSave(DisplayDataForm.value);
