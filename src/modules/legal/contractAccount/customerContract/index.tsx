...
import { customerColumns, prepayQueryForm, dataConversion, RootObjectChild } from '../use/index';
import { useDialog } from '@/use/dialog';
import { ListGenerallyTemplate } from '@gm/templates/list';
import { usePagination } from '@gm/hooks/ahooks';
import { QueryCustomerContract, UpdateContractTaxSubject } from '@/services/contract';
...
export default defineComponent({
  ...
  setup: (props, ctx) => {
    ...
    const query = usePagination(QueryCustomerContract, {
      transform: (res: any) => {
    ...
    const summaryMethod = ({ columns }: any) => {
      ...
      >([
        ...
        ['已收款金额 (元)', 'received_amount'],
        ['未收款金额 (元)', 'unreceived_amount'],
  ...
  render() {
    const columns = this.customerColumns(this);
    const keyEnter = () =>
      this.query.run(
        {
          page_num: 1,
