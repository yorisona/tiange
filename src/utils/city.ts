...
cityJson.forEach(province => {
  if (province.item_code.substr(2, 4) === '0000') {
    const citys = [];
    cityJson.forEach(city => {
      if (
        city.item_code !== province.item_code &&
        city.item_code.substr(0, 2) === province.item_code.substr(0, 2) &&
        city.item_code.substr(4, 2) === '00'
      ) {
        const countys = [];
        cityJson.forEach(county => {
          if (
            city.item_code === '110100' ||
            city.item_code === '120100' ||
            city.item_code === '310100' ||
            city.item_code === '500100'
          ) {
            if (
              county.item_code !== city.item_code &&
              county.item_code.substr(0, 3) === city.item_code.substr(0, 3) &&
              county.item_code.substr(4, 2) !== '00'
            ) {
              countys.push(county);
            }
          } else {
            if (
              county.item_code !== city.item_code &&
              county.item_code.substr(0, 4) === city.item_code.substr(0, 4) &&
              county.item_code.substr(4, 2) !== '00'
            ) {
              countys.push(county);
            }
        ...
        citys.push(Object.assign({}, city, { countys }));
      }
    });
    provinces.push(Object.assign({}, province, { citys }));
  }
});
