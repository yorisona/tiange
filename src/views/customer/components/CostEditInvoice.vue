<template>
  ...
  >
    <div>
      <el-form class="tax-point">
        <el-form-item label="税点金额" label-width="128px">
          <el-input
            v-model="tax_point"
            type="number"
            placeholder="请输入税点金额"
      ...
      >
        ...
        <el-form :rules="add_cost_form_rules" :model="item" :ref="`add_cost_form${item.id}`">
          ...
          <el-form-item label="发票号码:" label-width="128px" prop="invoice_num">
            <!--    -->
            <el-input
              maxlength="8"
          ...
          <el-form-item label="开票金额:" label-width="128px" prop="amount" class="amount">
            <el-input
              type="number"
              v-model="item.amount"
              placeholder="请输入开票金额"
...
export default {
  ...
  methods: {
    ...
    show(row) {
      ...
      this.file = '';
      // this.pic_url = "";
      this.$refs.costEditInvoiceDialog.dialogOpen();
      // 编辑
      if (row && row.invoice_info.length !== 0) {
        ...
        this.name = '编辑发票';
        // 循环表单对应赋值
        for (let index = 0; index < row.invoice_info.length; index++) {
          const _add_cost_form = {
            invoice_date: row.invoice_info[index].invoice_date, // 成本开票日期
            amount: row.invoice_info[index].amount, // 开票金额
            pic_url: row.invoice_info[index].pic_url, // 开票凭证
            institution: row.invoice_info[index].institution, //开票单位名称
            invoice_num: row.invoice_info[index].invoice_num, //开票号码
          };
          this.cost_info_list.push({
            ..._add_cost_form,
            ...{
      ...
      uploadCertificate(form)
        .then(data => {
          if (data.data && data.data.success) {
            ...
            for (let index = 0; index < this.cost_info_list.length; index++) {
              this.cost_info_list[index].pic_url = this.img_src_list[index];
            }
      ...
      if (
        ...
        saveCostInvoiceList({
          ...
          if (res.data.success) {
            ...
            this.$refs.costEditInvoiceDialog.dialogClose();
          } else {
